...
/*                                                                            */
/*  Both grounds are supported. On black the scene is additive — light piles   */
/*  up where tracks cross, which is how a real long exposure behaves. On a     */
/*  white ground that logic inverts: ink is laid down with source-over and     */
/*  every brightness in the palette becomes a darkness. See PALETTES.          */
/* -------------------------------------------------------------------------- */
...
/**
 * Everything the renderer needs to know about the ground it is drawing on.
 * Swapping this is what turns the scene from a long exposure into a plot.
 */
export type OrbitalPalette = {
  background: string;
  /** "lighter" piles light up on a dark ground; "source-over" lays ink on a light one. */
  blend: GlobalCompositeOperation;
  /** "hot" gives every body a white-hot core; "self" keeps it its own colour. */
  bodyCore: "hot" | "self";
  /** Base, cool and warm star tints, as "r,g,b". */
  star: [string, string, string];
  /** Overall star weight. Ink on white reads much heavier than light on black. */
  starAlpha: number;
  sunCore: string;
  sunHalo: string;
  /** Weight of the Sun's outer haze. Additive haze can be generous; ink cannot. */
  sunHaloAlpha: number;
  /** Sun-track gradient: the stop at the Sun, then the stop mid-way back. */
  track: [string, string];
  /** Colour of the scrim veil, as "r,g,b". */
  scrim: string;
};

export interface OrbitalHeroSectionProps
  extends React.HTMLAttributes<HTMLDivElement> {
  /** Bodies to run. Defaults to Mercury through Neptune with J2000 elements. */
  planets?: Planet[];
  /**
   * Which ground to draw on. "dark" is the original long exposure on black;
   * "light" inverts the whole paint layer to ink on paper. Note that the
   * default `planets` are tuned to glow on black — pass SOLAR_SYSTEM_LIGHT
   * (or your own colours) alongside theme="light".
   */
  theme?: "light" | "dark";
  /** Overrides on top of the chosen theme's palette. */
  palette?: Partial<OrbitalPalette>;
  ...
  focus?: [number, number];
  /**
   * Lays a veil over one edge so text can sit there and still be read. The
   * veil is drawn last, over the whole scene, fading out by two thirds of the
   * way across. Its colour comes from the palette, so on a light ground it
   * veils toward paper rather than toward black.
   */
  scrim?: "none" | "left" | "right" | "top" | "bottom";
  /** How opaque the veil gets at the edge it starts from, 0 to 1. */
  ...
  interactive?: boolean;
  /**
   * Element to read the pointer from. Defaults to the component's own host —
   * pass something else when the canvas sits behind other content and cannot
   * receive events itself.
   */
  pointerTarget?: React.RefObject<HTMLElement | null>;
  /** Freeze on the current frame. */
  paused?: boolean;
  /** Overrides the palette's Sun colour. */
  sunColor?: string;
...
const PALETTES: Record<"light" | "dark", OrbitalPalette> = {
  dark: {
    background: "#000000",
    blend: "lighter",
    bodyCore: "hot",
    star: ["255,255,255", "175,205,255", "255,214,170"],
    starAlpha: 1,
    sunCore: "#FFF2CC",
    sunHalo: "255,190,110",
    sunHaloAlpha: 1,
    track: ["255,246,214", "255,206,110"],
    scrim: "0,0,0",
  },
  /**
   * Ink on paper. Every brightness above becomes a darkness: the ground is
   * white, tracks are laid down with source-over instead of piling up
   * additively, and the Sun is an ink disc inside a blue aura rather than a
   * white-hot one. Star weight drops hard — a black speck on white carries
   * far more than a white speck on black, and at the original count the field
  ...
  light: {
    ...
    blend: "source-over",
    bodyCore: "self",
    // Slate and blue, not ink. A near-black speck on white reads as dirt on
    // the page; a soft grey one reads as depth, which is what the field is for.
    star: ["169,179,194", "127,158,196", "203,210,221"],
    starAlpha: 0.9,
    sunCore: "#051524",
    sunHalo: "32,117,255",
    sunHaloAlpha: 0.55,
    track: ["5,21,36", "32,117,255"],
    scrim: "255,255,255",
  },
};
...
/**
 * The same bodies, recoloured for a white ground: a blue-to-ink ramp rather
 * than a spectrum. The pastels above are chosen to glow additively on black
 * and all but vanish on paper. Earth keeps the one saturated accent.
 */
export const SOLAR_SYSTEM_LIGHT: Planet[] = SOLAR_SYSTEM.map((p, i) => ({
  ...p,
  color: ["#a9b3c2", "#8b96a8", "#2075ff", "#6b7280", "#1a66e0", "#445062", "#7f9ec4", "#051524"][i] ?? p.color,
  glow: p.name === "Earth" ? 1.2 : 1,
}));
...
export function OrbitalHeroSection({
  ...
  const pal: OrbitalPalette = { ...PALETTES[theme], ...paletteOverride };
  const bodies = planets ?? (theme === "light" ? SOLAR_SYSTEM_LIGHT : SOLAR_SYSTEM);

  const config = {
    planets: bodies, pal, yearSeconds, trailYears, compress, maxTurns, planeSpread, eccentricity,
    alignToCourse, driftSpeed, apex, viewRadius, tilt, spin, roll, lead, focus, scrim, scrimStrength,
    starCount, glow, showOrbits, showSunTrack, interactive, pointerTarget, paused,
    sunColor: sunColor ?? pal.sunCore,
  };

  // The animation loop is set up once and then reads live props off this ref,
  // so a prop change never tears the loop down. The ref is written from an
  // effect rather than during render — writing it inline would mutate during
  // render, which React forbids — and the useRef initialiser covers the first
  // frame, which is painted before any effect runs.
  ...
  useEffect(() => {
    ...
    /* --- sprites ---------------------------------------------------------- */
    // Keyed by colour AND core mode: the same planet needs a white-hot centre
    // on black and a solid ink one on paper.
    const glowCache = new Map<string, HTMLCanvasElement>();
    function glowSprite(color: string, core: "hot" | "self"): HTMLCanvasElement {
      ...
      grad.addColorStop(0, core === "hot" ? "rgba(255,255,255,1)" : `rgba(${r},${g},${b},0.92)`);
    ...
    // The canvas often sits behind the copy it backs, where it never sees a
    // pointer event of its own — so the source is a prop, defaulting to us.
    ...
    function drawSun(k: number, t: number) {
      ...
      const [hr, hg, hb] = parseRGB(pal.sunHalo);
      const ha = pal.sunHaloAlpha;
      ...
      haze.addColorStop(0, `rgba(${hr},${hg},${hb},${0.05 * k * ha})`);
      haze.addColorStop(0.4, `rgba(${hr},${hg},${hb},${0.014 * k * ha})`);
      ...
      outer.addColorStop(0, `rgba(${r},${g},${b},${0.34 * k * ha})`);
      outer.addColorStop(0.3, `rgba(${hr},${hg},${hb},${0.1 * k * ha})`);
      outer.addColorStop(0.62, `rgba(${hr},${hg},${hb},${0.025 * k * ha})`);
      outer.addColorStop(1, `rgba(${hr},${hg},${hb},0)`);
      ctx!.fillStyle = outer;
      ctx!.beginPath();
      ctx!.arc(cx, cy, R * 4.6, 0, TAU);
      ctx!.fill();

      const hot = pal.bodyCore === "hot";
      const bloom = ctx!.createRadialGradient(cx, cy, 0, cx, cy, R * 2.3);
      bloom.addColorStop(0, hot ? `rgba(255,255,255,${k})` : `rgba(${r},${g},${b},${0.9 * k})`);
      bloom.addColorStop(0.42, `rgba(${r},${g},${b},${0.7 * k})`);
      bloom.addColorStop(0.72, `rgba(${hr},${hg},${hb},${0.22 * k})`);
      ...
      ctx!.fillStyle = hot ? "rgba(255,255,255,1)" : `rgb(${r},${g},${b})`;
      ctx!.beginPath();
    ...
    function render(t: number) {
      ...
      for (let s = 0; s < starN; s++) {
        ...
        let a = (0.2 + sMag[s] * 1.05) * Math.pow(dRef / P.depth, 0.8) * near * far * pal.starAlpha;
        if (a <= 0.012) continue;
        a *= 0.82 + 0.18 * Math.sin(t * 9 + sPhase[s]);
        const col = sTint[s] === 1 ? pal.star[1] : sTint[s] === 2 ? pal.star[2] : pal.star[0];
      ...
      if (C.showSunTrack) {
        ...
        if (P.ok) {
          ...
          grad.addColorStop(0, `rgba(${pal.track[0]},${k})`);
          grad.addColorStop(0.45, `rgba(${pal.track[1]},${0.55 * k})`);
          grad.addColorStop(1, `rgba(${pal.track[1]},0)`);
      ...
      const drawShot = (o: Shot) => {
        ...
        const R = size * 3.3;
        const [r, g, b] = o.el.rgb;
        ctx!.globalAlpha = Math.min(1, 0.9 * bright);
        ctx!.drawImage(glowSprite(o.el.p.color, pal.bodyCore), o.x - R, o.y - R, R * 2, R * 2);
        ctx!.globalAlpha = 1;
        ctx!.fillStyle = pal.bodyCore === "hot" ? "rgba(255,255,255,0.95)" : `rgba(${r},${g},${b},0.95)`;
        ctx!.beginPath();
      ...
      if (C.scrim !== "none") {
        ...
        // Heavy at the edge, then off quickly — a straight ramp would veil the
        ...
        // eye picks the kink out as a faint band.
        for (let q = 0; q <= 12; q++) {
          const x = q / 12;
          g.addColorStop(x, `rgba(${pal.scrim},${(s * Math.pow(1 - x, 2.4)).toFixed(4)})`);
        }
        ctx!.fillStyle = g;
        ctx!.fillRect(0, 0, width, height);
      }
    ...
    pointerHost.addEventListener("pointermove", onPointer);
